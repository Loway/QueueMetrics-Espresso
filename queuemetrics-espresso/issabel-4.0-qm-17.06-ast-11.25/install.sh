...
ROOTPASSWD=`awk -F'=' '/^mysqlrootpwd/ {print $2}' /etc/issabel.conf`
...
echo "Issabel - Installing $0 from $PWD";
echo "File: $FILESQL"
...
rv $CPROPSQM cluster.servers issabel

rv $CPROPSQM default.crmapp "http://www.queuemetrics.com/sample_screen_pop.jsp?agent=[A]\&unique=[U]"

add $CPROPSQM cluster.issabel.manager tcp:qmmanager:${AMIPASSWD}@127.0.0.1
add $CPROPSQM cluster.issabel.queuelog sql:P001
add $CPROPSQM cluster.issabel.monitored_calls /var/spool/asterisk/monitor/
add $CPROPSQM cluster.issabel.callfile.dir tcp:qmmanager:${AMIPASSWD}@127.0.0.1
